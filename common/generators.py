...
class UnchunkedGenerator:
    ...
    def next_epoch(self):
        for seq_cam, seq_3d, seq_2d in zip_longest(self.cameras, self.poses_3d, self.poses_2d):
            ...
            #at evaluation time only un-comment
            #miss = 1 # random miss joints
            #t1,t2,t3=seq_2d.shape
            #tempPose2d=np.ones((t1,t2,t3*2))
            #tempPose2d[:,:,0]=seq_2d[:,:,0]
            #tempPose2d[:, :, 2] = seq_2d[:, :, 1]

            #for p_x in range(t1):  # for complete seq
            #    indices_r = random.sample(range(0,t2), miss)#np.random.randint(0, 17, miss)
            #    for ind in range(t2):
            #        #iny = indices_r[ind]
