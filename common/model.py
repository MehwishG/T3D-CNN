...
class TemporalModelBase(nn.Module):
    ...
    def __init__(self, num_joints_in, in_features, num_joints_out,
        ...
        print("fil width", filter_widths[0])
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1)
        #print("x shape before fwd block ",x.shape)
...
class TemporalModel(TemporalModelBase):
    ...
    def __init__(self, num_joints_in, in_features, num_joints_out,
        ...
        print("filter_width.shape",np.asarray(filter_widths).shape)
        ...
        for i in range(1, len(filter_widths)):
            ...
            print("next di fac",next_dilation)
    ...
    def _forward_blocks(self, x):
        ...
        #print("length of self pad",len(self.pad))
        #print("before padding",x.shape)
        for i in range(len(self.pad) - 1):
            pad = self.pad[i+1]
            shift = self.causal_shift[i+1]
            #print(" padding and shifting",pad,"next  ",shift)
            res = x[:, :, pad + shift : x.shape[2] - pad + shift]
            #print("slicing",res.shape)
            x = self.drop(self.relu(self.layers_bn[2*i](self.layers_conv[2*i](x))))
            x = res + self.drop(self.relu(self.layers_bn[2*i + 1](self.layers_conv[2*i + 1](x))))
            #print("after paddingx x after skip con",x.shape)
        x = self.shrink(x)
