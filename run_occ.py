    ...
    for subject in subjects:
        for action in keypoints[subject].keys():
            if action_filter is not None:
                ...
                if not found:
                    continue
            #### train part comment during testing    
            poses_2d = keypoints[subject][action]
            poses_2d_n=[]
            for p1 in range(len(poses_2d)):
                t1, t2, t3 = poses_2d[p1].shape
                tempPose2d=np.ones((t1,t2,4))
                tempPose2d[:,:,0]=poses_2d[p1][:,:,0]
                tempPose2d[:, :, 2] = poses_2d[p1][:, :, 1]
                
                miss=1#random miss joints for traing only and for test check unchunked generator
                for p_x in range(t1): #for complete seq
                    indices_r=random.sample(range(0,t2), miss)
                    for ind in range(t2):
			if ind not in indices_r:
                            tempPose2d[p_x,ind,:]=0.
                poses_2d_n.append(tempPose2d)
            poses_2d=poses_2d_n
            #####################################
        ...
        with torch.no_grad():
            ...
            if not args.no_eval:
                ...
                for cam, batch, batch_2d in test_generator.next_epoch():
                    ...
                    #print(inputs_2d.shape,"so chalo check")# Predict 3D poses
                ...
                for cam, batch, batch_2d in train_generator_eval.next_epoch():
                    ...
                    # Compute 3D poses
                    #print("sape of nput",inputs_2d.shape)
                    predicted_3d_pos = model_pos(inputs_2d)
                    loss_3d_pos = mpjpe(predicted_3d_pos, inputs_3d)
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False):
    ...
    with torch.no_grad():
        ...
        for _, batch, batch_2d in test_generator.next_epoch():
            ...
            #print("so shpaeee is ",inputs_2d.shape)# Positional model
            if not use_trajectory_model:
                predicted_3d_pos = model_pos(inputs_2d)
            else:
...
if args.render:
    ...
    if args.viz_output is not None:
        ...
        anim_output = {'T3D CNN': prediction}
        if ground_truth is not None and not args.viz_no_ground_truth:
            anim_output['Ground truth'] = ground_truth
        
        input_keypoints = image_coordinates(input_keypoints[..., :2], w=cam['res_w'], h=cam['res_h'])
        from common.visualization_ccOld import render_animation  #for random
    ...
    def run_evaluation(actions, action_filter=None):
        ...
        for action_key in actions.keys():
            ...
            gen = UnchunkedGenerator(None, poses_act, poses_2d_act,
                                     ...
                                     kps_left=kps_left, kps_right=kps_right, joints_left=joints_left, joints_right=joints_right)
            e1, e2, e3, ev,err_lst = evaluate(gen, action_key)
            errors_p1.append(e1)
            errors_p2.append(e2)
            errors_p3.append(e3)
